class MiqRequest < ApplicationRecord
  ...
  def pending
#    self.state = "pending"
#    update_attributes(:approval_state => "pending_approval", :request_state => "pending", :state => "pending", :status => "Ok")
    call_automate_event_queue("request_pending")
 #   call_automate_event("request_pending", :synchronous => true)
    update_attributes(:approval_state => "pending_approval", :request_state => "pending", :state => "pending", :status => "Ok")
#    miq_approvals.first.state="pending"
#    parent = miq_request
#    parent.reload
  end

#  def crocer_pending
#    update_attributes(:request_state => "crocer")
#    miq_request.reload
#  end
  ...
  def approval_approved
    ...
    puts "VISH_DEBUG_ENGINE #{miq_request.get_tag(:approvestage).to_i}"
    if miq_request.get_tag(:approvestage).to_i == 1
